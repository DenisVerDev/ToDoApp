...
2. I was relying too much on previous iteration of this project. After thinking for a bit I came to conclusion that service methods should not return anything other than a status code, of course the exception be fetching methods. The thing is, why should I return more data for UpdateTaskContentAsync for example? Place from which request that came my way already knows what changes there are and is only awaiting confirmation of a successful operation. If needed, client can send a fetch request after.
3. I need service for user authentication and registration. Deletion too.